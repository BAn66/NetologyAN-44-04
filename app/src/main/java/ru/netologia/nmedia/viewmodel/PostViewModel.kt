...
class PostViewModel(application: Application, myRepository: PostRepository) :
    ...
    private val repository: PostRepository = myRepository
    ...
    fun load() { //Загружаем посты c помщью коротюнов и вьюмоделскоуп
        _data.value = FeedModelState(loading = true)
        viewModelScope.launch {
            val posts = repository.getAll()
            _data.value = FeedModelState(posts = posts, empty = posts.isEmpty())
        }
    }
